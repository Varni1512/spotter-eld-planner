...
export default function StopsTimeline({ stops = [], isPlanned }) {
                  ...
                  <div className="flex-1 bg-slate-50/70 hover:bg-slate-50 border border-slate-200/80 rounded-xl p-4 sm:p-4.5 transition-all">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1.5 pb-2 border-b border-slate-200/60">
                      ...
                      <div className="text-xs text-slate-600 font-medium">
                        {stop.mile_marker > 0 ? (
                          <span>Route Mile: <strong className="text-slate-800">{stop.mile_marker} mi</strong></span>
                        ) : (
                          <span className="text-slate-400">Departure origin (Mile 0.0)</span>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-3">
                      <div>
                        <div className="text-xs text-slate-700 flex items-start gap-1.5 font-medium">
                      ...
                      <div className="bg-white rounded-lg border border-slate-200/70 p-2.5 text-xs flex flex-col justify-between">
                        <div className="flex items-center justify-between text-slate-700 mb-1">
                          <span className="font-semibold text-slate-900">{stop.reason || 'Operational Stop'}</span>
                          <span className="text-[11px] font-medium text-slate-600 px-1.5 py-0.5 bg-slate-100 rounded">
                            {stop.duration_minutes >= 60
                              ? `${(stop.duration_minutes / 60).toFixed(1)} hrs`
